...
unsigned int NewPing::ping_median(uint8_t it) {
	...
	while (i < it) {
		...
		if (i < it) delay(PING_MEDIAN_DELAY - (last >> 10)); // Millisecond delay between pings. **********DELAY IS HERE****
	}
...
uint16_t NewPing::getDistance(Unit _unit){
	if( (micros() - _lastTime) > 50000UL){
		_lastTime = micros();
		if(_unit == CM)
			_sonarRange =  (ping() / US_ROUNDTRIP_CM);	//return centimeters.
		else if(_unit == IN)
			_sonarRange = (ping() / US_ROUNDTRIP_IN);	//return inches.
		else if(_unit == FT)
			_sonarRange = (ping() / (US_ROUNDTRIP_IN * 12));	//return feet.
	}
	return _sonarRange;
}
